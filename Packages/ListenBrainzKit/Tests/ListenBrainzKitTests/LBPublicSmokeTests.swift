...
struct LBPublicSmokeTests {
    ...
    func publicReadSurface() async throws {
        ...
        try await pace()
        let artistMBID = try #require(artists?.artists.compactMap(\.mbid).first)
        let popularity = try await client.popularity.artists([artistMBID])
        try await pace()
        let activity = try await client.stats.listenActivity(user: username, range: .allTime)
        try await pace()
        _ = try await client.pins.current(user: username)
        try await pace()
        let pins = try await client.pins.history(user: username, count: 2)
        try await pace()
        ...
        #expect(popularity.first?.mbid == artistMBID)
        #expect(popularity.first?.totalListenCount != nil || popularity.first?.totalUserCount != nil)
        #expect(activity?.activity.isEmpty == false)
        #expect(pins.totalCount >= pins.pinnedRecordings.count)
