...
| API/domain | ListenBrainzKit | Existing P0 clients/models | Reliability fixes, app adapters, typed CF/social/pins/detail/era/evolution-stat/generated-radio extensions |
...
| Playlist create/edit | Current LB server/API + official Android/KMP behavior + ListenBrainzKit | Exact JSPF/full-snapshot semantics, private-by-default behavior, collaborator normalization, and MPL transport patterns; no GPL code copied | Typed Kit requests, one-attempt shared-gate provider, account-scoped cache reconciliation, and native create/edit forms |
...
| Year in Music | Cassette Wrapped + current LB API/web + official Android sharing behavior | MPL mesh hero, ranked artist shelf, album grid, and compact track-list composition; current web report/art semantics and SVG-to-image sharing behavior | One gated aggregate report, UTC annual heatmap, native entity navigation, independent report-link sharing, explicit request-gated SVG document preview, system PNG sharing, and accessibility-responsive story |
| LB Radio | Current LB server/web + ListenBrainzKit JSPF + official-client behavior | Current prompt/mode/schema semantics, existing MPL playlist parsing, and the app's native playlist mosaic/rows; no GPL code copied | Typed generator extension, safe native presets plus advanced prompt, explicit gated generation, one optional batch enrichment, honest browse-only result screen, and adaptive native SwiftUI |
