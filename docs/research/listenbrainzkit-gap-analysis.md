...
| Playlist create/edit | SUPPORTED | Typed exact-JSPF create and full-snapshot metadata edit, including privacy/collaborators and endpoint-specific errors | Reuse through the shared gate; local MPL extension is an upstream candidate |
| Playlist delete/items/copy/import/export | MISSING | Delete, add/remove/reorder, copy, import, and export remain absent | Add only with a concrete native workflow and mutation-specific reconciliation |
