...
| Playlist create/edit/add items | SUPPORTED | Typed exact-JSPF create, full-snapshot metadata edit, and append for 1...100 canonical recording MBIDs, including endpoint-specific errors and strict success-envelope validation | Reuse through the shared gate; local MPL extensions are upstream candidates; native UI currently appends one item |
| Playlist delete/remove/reorder/copy/import/export | MISSING | Delete, position-based remove/reorder, copy, import, and export remain absent | Add only with a concrete native workflow and mutation-specific reconciliation |
