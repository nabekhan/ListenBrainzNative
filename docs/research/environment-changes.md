...
- `/tmp/listenbrainz-xcodebuild.log`: transient command-line build log. Cleanup: remove that exact file.
- `/tmp/listenbrainz-testbuild.log`: transient command-line test-build log. Cleanup: remove that exact file.
- `/tmp/listenbrainz-tests.log`, `/tmp/brainz-*.png`, and `/tmp/brainz-*-boot.log`: transient simulator test logs, visual checkpoints, and boot logs. Cleanup: remove those matching temporary files.
