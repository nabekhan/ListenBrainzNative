...
- External source actions use app-owned `ExternalMediaLink` validation and native SwiftUI `Link`. ListenBrainz web/server GPL sources supplied provider-label behavior only; no source or UI was copied. Cassette's MPL patterns remain unrelated to the native Link implementation.
