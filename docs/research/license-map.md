...
- Generic stats, artist, and playlist artwork reuse that independently written native renderer/share flow with app-owned SwiftUI and a local MPL-2.0 ListenBrainzKit transport extension. The GPL server/website and official clients supplied endpoint behavior and resource-shape evidence only; no GPL source or UI was copied.
...
- Single-item playlist reorder UI, reconciliation, and shared durable item-mutation recovery are independently written from current API/server/web behavior and native SwiftUI. Its typed move transport is a local MPL-2.0 ListenBrainzKit extension; official GPL Android/KMP code was reference-only, no donor UI was copied, and no new dependency was added.
...
- CritiqueBrainz review highlights and the value-only full-text reader use current website request behavior and official Android/KMP response-field semantics as reference only. The KMP source is GPL/reference-only; the native client, decoder, cache, and presentation are independently implemented, with no new dependency or attribution requirement.
