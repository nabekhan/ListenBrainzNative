...
The current 2026-09-26 run packaged app source at product commit `6e77b3f` from clean checkpoint `97b970a`. It produced `Brainz-0.1.0-1-97b970a-unsigned.ipa` at 7,084,837 bytes with SHA-256 `639867b9e1212370d86df4030247933c536c52346442a83c85c7e83d9a9f0521`. Independent extraction found exactly 12 unique entries under `Payload/Brainz.app`, one expected unsigned thin arm64 iOS executable, bundle `dev.nabekhan.listenbrainznative`, version `0.1.0 (1)`, minimum iOS 18, only Apple system dependencies, and a privacy manifest byte-identical to source. It found no traversal or duplicate ZIP entry, symlink, profile, code-signature command or directory, nested code container, unexpected executable, or extra top-level path. The binary contains the new public-profile cache and saved-profile UI markers, which are absent from the older ignored IPA. A separate security review reports SHIP for this unsigned/re-signable boundary. The IPA and the superseded ignored artifact were removed after recording evidence and can be reproduced with the script; external signing and physical-device checks remain required.
