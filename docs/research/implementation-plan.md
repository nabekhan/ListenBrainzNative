...
- Phase 3: full history/date jump, feed mutations, recommendation actions, and broader playlist browsing. The native read-only My Feed/Following/Similar experience, Statistics, Fresh Releases, scoped search, Pins, visited-user/social profiles, release-group/playlist details, and For You recommendations now have initial slices.
...
1. Extend the social feed with recommendation/thanks/hide actions only after read-only feed QA is complete.
...
- The verified checkpoint currently passes 76 vendored-package tests and 75 app tests, plus the deliberately paced opt-in production public API smoke suite.
