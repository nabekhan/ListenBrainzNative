...
- `ArtistService` and `ArtistPayload` already model the public `POST artist/{artist_mbid}` page response, including `popularRecordings`, popularity-ranked `releaseGroups`, and ranked `similarArtists`. The shared `ArtistViewModel` projects those into popular tracks, albums/appears-on, and similar-artist UI state. This confirms official-client behavior, but the response is a broad internal page contract rather than a small documented API and does not justify importing the KMP framework solely for these shelves.
