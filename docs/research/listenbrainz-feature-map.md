...
| User search and visited-user profiles | Y | Y | Y | Y | Y | Y | P1 | Native scoped search/profile landed with staged overview, independently lazy all-time artist, release, and track sections, plus explicit Playlists and Year in Music destinations; typed public sections persist anonymously for offline/stale-first reuse, while Playing Now remains live-only and no destination request starts until it is opened |
...
| Reviews/CritiqueBrainz events | Y | Y | Y | Y | — | Y | P2 | Native highlights and an explicit, bounded, cached full-text reader landed for published entity reviews; opening is request-free, pages load only after a tap, and writing/voting remains staged |
...
| User/collaborator playlists | Y | Y | Y | Y | Y | Y | P1 | Native owned/collaborating pages open from self and visited profiles with lazy server pagination; owners can create and edit full metadata/collaborator snapshots through exact debounced user search, while collaborators retain track-only authority |
...
| Playlist item remove/reorder | Y | Y | Y | ? | Y* | Y | P2 | Safe single-item and contiguous-range removal plus one-track positional reordering landed with fresh identity/order verification, one no-retry POST, durable shared recovery, and exact postflight confirmation. `*` uses local MPL Kit extensions. Multi-item move remains intentionally unavailable because the current server implements it as non-atomic delete-then-add and official clients use only `count: 1` |
| Playlist import/export/service sync | Y | Y | P | ? | — | P | P3 | Native request-free export of the already-loaded playlist to a bounded JSPF JSON snapshot landed; service import/export and synchronization remain staged |
...
| Shareable art/stat grids/YiM art | Y | Y | P | P | Y* | P | P1 | Explicit native stats, artist, playlist, YiM, and ordered top-album collage creation/PNG sharing landed; `*` includes the local upstreamable custom-grid Kit extension |
...
- Generic Art now uses typed public stats/artist reads, the authenticated playlist POST, and the public ordered-release custom-grid POST. The native Album collage composer operates only on already-loaded canonical top releases; its draft type carries no remote artwork URL, so open, selection, layout, style, reordering, and dismissal are request-free. **Create** freezes one exact request. Every image is otherwise created only after a user action, exact requests coalesce through the shared gate, public calls omit the token, private playlist cache entries stay credential-scoped, unavailable results are cached, and raw SVG retention is bounded by both age and 16 MiB total. The shared nonpersistent renderer validates every external SVG resource before loading, blocks all other WebKit traffic, and exports a native PNG; a single public artist smoke rendered the current Archive redirect and placeholder shapes without an account token.
...
- Visited profiles use only anonymous public reads and a bounded typed cache keyed by canonicalized username hash. Overview and ranking sections expire independently, cache access cannot extend their seven-day usable retention, stale values refresh once without hiding useful content, and current Playing Now state is neither written to disk nor retained across a failed refresh. Tokens, private playlists, and account-only responses are outside this cache boundary.
